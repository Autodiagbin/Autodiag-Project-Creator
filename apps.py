...
class Ui_main_window(object):
        ...
        # Specificité  
                ...
                self.all_specs = [self.cb_spec1,self.cb_spec2,self.cb_spec3,self.cb_spec4,self.cb_spec5,self.cb_spec6,self.cb_spec7,self.cb_spec8,self.cb_spec9,self.cb_spec10]
                ...
                self.all_options = [self.cb_option1,self.cb_option2,self.cb_option3,self.cb_option4,self.cb_option5,self.cb_option6]
        ...
        def stylize(self):
                ...
                # flags = QtCore.Qt.WindowFlags(QtCore.Qt.WindowType.FramelessWindowHint)
                # main_window.setWindowFlags(flags)
        
                # self.grp_windows_btn = QtWidgets.QGroupBox(main_window)
                # self.grp_windows_btn.setGeometry(QtCore.QRect(475, 5, 60, 20))
                # self.grp_windows_btn.setObjectName("grp_windows_btn")

                # self.btn_close = QtWidgets.QPushButton(self.grp_windows_btn)
                # self.btn_close.setGeometry(QtCore.QRect(30, 0, 30, 20))
                # self.btn_close.setObjectName("btn_proceed")
                # self.btn_close.setText("✕")
                # self.btn_close.clicked.connect(self.close)
                # self.btn_close.setStyleSheet(f"background-color: {color_btn}; color: {color_bg2}")

                # self.btn_reduce = QtWidgets.QPushButton(self.grp_windows_btn)
                # self.btn_reduce.setGeometry(QtCore.QRect(0, 0, 30, 20))
                # self.btn_reduce.setObjectName("btn_proceed")
                # self.btn_reduce.setText("–")
                # self.btn_reduce.clicked.connect(self.reduce)
                # self.btn_reduce.setStyleSheet(f"background-color: {color_btn2}; color: {color_bg2}")

                # self.head_windowHead = QtWidgets.QGroupBox(main_window)
                # self.head_windowHead.setGeometry(QtCore.QRect(10, 0, 460, 25))
                # self.head_windowHead.setObjectName("head_windowHead")
                # #self.head_windowHead.setStyleSheet("background-color : red")


                # self.lab_windowTitle = QtWidgets.QLabel(self.head_windowHead)
                # self.lab_windowTitle.setGeometry(QtCore.QRect(0, 0, 460, 25))
                # self.lab_windowTitle.setText(f" Autodiag Project Creator - V{self.version}")

                main_window.setStyleSheet(f"background-color: {color_bg}; color: {color_secondary}")
                self.tabs.setStyleSheet("QLineEdit {background-color: #7b7477 }" + "\n" + "QCheckBox::indicator::disabled { background: none}")
        ...
        def proceed(self):
                ...
                if (self.cbb_autodiag.currentText() and self.cbb_marque.currentText() and self.cbb_locate.currentText() and self.input_model.text()) != "" and (self.cbb_autodiag.currentText() and self.cbb_marque.currentText() and self.cbb_locate.currentText() and self.input_model.text()) != None:
                        ...
                        new_project = project.Project(marque=self.cbb_marque.currentText(),modele=modele_suffixed,autodiag=self.cbb_autodiag.currentText(),path=dir,specifite=spec_validated,options=options_validated,printcpt=self.cbb_locate.currentText(),version=self.input_version.text(),type_objet=type_objet)
