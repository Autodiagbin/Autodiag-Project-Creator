...
class AutodiagProjectManager(object):
    ...
    def setup_ui(self, windows_main):
        windows_main.setObjectName("main_window")
        windows_main.resize(555, 630)
        windows_main.setMinimumSize(QtCore.QSize(555, 630))
        windows_main.setMaximumSize(QtCore.QSize(555, 630))
        windows_main.setStyleSheet("")
        ...
        self.retranslate_ui(windows_main)
        self.populate()
        self.tabs.setCurrentIndex(0)
        self.stylize()
        QtCore.QMetaObject.connectSlotsByName(windows_main)

    def retranslate_ui(self, window_main):
        _translate = QtCore.QCoreApplication.translate
        window_main.setWindowTitle(_translate("main_window", "Autodiag - Création de projet"))
    ...
    def proceed(self):
        ...
        if (self.cbb_autodiag.currentText() and
            self.cbb_marque.currentText() and
            self.cbb_locate.currentText() and
            self.input_model.text()) != "" and (self.cbb_autodiag.currentText() and self.cbb_marque.currentText() and
                                                self.cbb_locate.currentText() and self.input_model.text()) is not None:
            ...
            new_project = project.Project(marque=self.cbb_marque.currentText(),
                                          modele=modele_suffixed,
                                          autodiag=self.cbb_autodiag.currentText(),
                                          path=directory, specifite=spec_validated,
                                          options=options_validated,
                                          printcpt=self.cbb_locate.currentText(),
    ...
    def push_history(self, project_name):
        DB.insert(project_name.__dict__)
    ...
    def get_history(self, search=None):
        if search is None or search == "" or search is False:
            historique = DB.all()
        else:
    ...
    def update_info(self, info, statut="info"):
        if statut == "warning":
